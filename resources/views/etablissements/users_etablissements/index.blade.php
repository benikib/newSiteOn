...
@section('content')
    ...
    <div class="container py-4">
      ...
      <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <!-- Modal Ajout -->
  @include('etablissements.users_etablissements.create')
                    ...
                    <div class="card-body px-0 pt-0 pb-2">
                        <div class="table-responsive p-0">
                            <table class="table align-items-center justify-content-center mb-0">
                                ...
                                <tbody>
                                    @foreach ($userEtablissements as $ets)
                                        <tr>
                                            <td class="text-xs font-weight-bold">{{ $loop->iteration }}</td>
                                            <td class="text-xs font-weight-bold">{{ $ets->user->name }}</td>
                                            <td class="text-xs font-weight-bold">{{ $ets->user->email }}</td>
                                            <td class="text-xs font-weight-bold">{{ $ets->user->telephone }}</td>

                                            {{-- <td class="align-middle">
                                                <a href="#" class="text-secondary font-weight-bold text-xs"
                                                    data-bs-toggle="modal" data-bs-target="#editModal{{ $ets->id }}">
                                                    Edit
                                                </a>
                                            </td> --}}
                                        </tr>
                            ...
                            @include('admins.users_etablissements.edit') --}}
                        </div>
                    </div>
                </div>
            </div>
