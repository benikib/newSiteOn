                        ...
                        <div class="col-4">
                            <div class="dropdown text-end mb-6">
                                ...
                                <ul class="dropdown-menu px-2 py-3" aria-labelledby="dropdownUsers1">
                                    <li><a class="dropdown-item border-radius-md" href="{{ route('users.index') }}">Voir tous</a></li>
                                </ul>
                            </div>
                            @php
                                $newUsersLastMonth = $users->filter(function($user) {
                                    return $user->created_at >= now()->subMonth();
                                })->count();
                                $percentage = $users->count() ? round(($newUsersLastMonth / $users->count()) * 100) : 0;
                            @endphp
                            <p class="text-dark text-sm text-end font-weight-bolder mt-auto mb-0">+{{ $percentage }}%</p>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="col-4">
                            <div class="dropstart text-end mb-6">
                                ...
                                <ul class="dropdown-menu px-2 py-3" aria-labelledby="dropdownUsers2">
                                    <li><a class="dropdown-item border-radius-md" href="{{ route('etablissements.index') }}">Voir tous</a></li>
                                </ul>
                            </div>
                            @php
                                $newEtabLastMonth = $etablissements->filter(function($etab) {
                                    return $etab->created_at >= now()->subMonth();
                                })->count();
                                $percentage = $etablissements->count() ? round(($newEtabLastMonth / $etablissements->count()) * 100) : 0;
                            @endphp
                            <p class="text-dark text-sm text-end font-weight-bolder mt-auto mb-0">+{{ $percentage }}%</p>
                        </div>
                    </div>
            ...
            <div class="card">
                <div class="card-body p-3 position-relative">
                    <div class="row">
                        <div class="col-8 text-start">
                            <div class="icon icon-shape bg-white shadow text-center border-radius-2xl">
                                <i class="ni ni-notification-70 text-dark text-gradient text-lg opacity-10" aria-hidden="true"></i>
                            </div>
                            <h5 class="text-dark font-weight-bolder mb-0 mt-3">
                                {{ $publicites->count() }}
                            </h5>
                        ...
                        <div class="col-4">
                            <div class="dropdown text-end mb-6">
                                ...
                                <ul class="dropdown-menu px-2 py-3" aria-labelledby="dropdownUsers3">
                                    <li><a class="dropdown-item border-radius-md" href="{{ route('publicites.index') }}">Voir tous</a></li>
                                </ul>
                            </div>
                           @php
    $activePub = $publicites->filter(function($pub) {
        if (!$pub->date || !$pub->dure) return false; // Skip if missing data
        $date = \Carbon\Carbon::parse($pub->date);
        return $date <= now() && $date->copy()->addDays($pub->dure
        ) >= now();
    })->count();
    $percentage = $publicites->count() ? round(($activePub / $publicites->count()) * 100) : 0;
@endphp
                            <p class="text-dark text-sm text-end font-weight-bolder mt-auto mb-0">{{ $percentage }}% actives</p>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="col-4">
                            <div class="dropstart text-end mb-6">
                                ...
                                <ul class="dropdown-menu px-2 py-3" aria-labelledby="dropdownUsers4">
                                    <li><a class="dropdown-item border-radius-md" href="{{ route('type_etablissements.index') }}">Voir tous</a></li>
                                </ul>
                            </div>
                            @php
                                $mostCommonType = $typeEtablissements->sortByDesc(function($type) use ($etablissements) {
                                    return $etablissements->where('type_etablissement_id', $type->id)->count();
                                })->first();
                                $count = $mostCommonType ? $etablissements->where('type_etablissement_id', $mostCommonType->id)->count() : 0;
                            @endphp
                            <p class="text-dark text-sm text-end font-weight-bolder mt-auto mb-0">{{ $count }} top</p>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <table class="table align-items-center mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Date création</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="text-xs font-weight-bold">{{ $etablissement->typeEtablissement->nom ?? 'N/A' }}</span>
                                    </td>
                                    <td>
                                        <span class="text-xs font-weight-bold">{{ $etablissement->adresse }}</span>
                                    </td>
                                    <td>
                                        <span class="text-xs font-weight-bold">{{ $etablissement->created_at->format('d/m/Y') }}</span>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
                        ...
                        <table class="table align-items-center mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Établissement</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="text-xs font-weight-bold">{{ $publicite->etablissement->nom }}</span>
                                    </td>
                                    <td>
...
<script>
    ...
    const typeCounts = {{ Js::from($typeEtablissements->map(function($type) use ($etablissements) {
        return $etablissements->where('type_etablissement_id', $type->id)->count();
    })) }};
